# compressed_SIDH 3DLP with small storage
This code base is an attachment of the manuscript "An Alternative Approach for Computing Discrete Logarithms in Compressed SIDH", utilizing several techniques to compute only 3 discrete logarithms with small storage during public-key compression. In particular, the code measures the time required for public-key compression (except the dual isogeny computation).
...
Algorithm 3, 4, 5, 7, 8 are implemented in [`dlog.c`](src/compression/dlog.c), which are called by function `EphemeralKeyGeneration_A_new` and `EphemeralKeyGeneration_B_new` in [`sidh_compressed.c`](src/compression/sidh_compressed.c) to realize the public key compression process. The strategies used by the new algorithms is stored in [`strategy.h`](src/compression/strategy.h).
