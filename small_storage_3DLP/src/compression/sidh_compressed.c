...
unsigned long long cycles_compress_A_new;
unsigned long long cycles_compress_B_new;
#endif
...
#define _TEST_r (_TEST_m % _TEST_w)
	...
	int label = 0;
	choose_base(_TEST_l, _TEST_m, _TEST_w, f[0], f[1], &label, _A);
#if _TEST_r != 0
#define _TEST_solveDLP(x) (PH_DLP2((x), D, Str_, _TEST_m, _TEST_l, _TEST_w, B, f[label],_A))
#else
#define _TEST_solveDLP(x) (PH_DLP((x), D, Str_, _TEST_m, _TEST_l, _TEST_w, B, _A))
#endif

    f2elm_t B[POWER(_TEST_l,_TEST_w)];
...
#ifndef SIKETEST
    cycle2 = _cpucycles();
    cycles_compress_A_new = cycles_compress_A_new + (cycle2 - cycle1);
#endif
	return 0;
#undef _TEST_l
#undef _TEST_m
...
#ifndef SIKETEST
    cycle2 = _cpucycles();
    cycles_compress_B_new = cycles_compress_B_new + (cycle2 - cycle1);
#endif
	return 0;
#undef _TEST_l
#undef _TEST_m
