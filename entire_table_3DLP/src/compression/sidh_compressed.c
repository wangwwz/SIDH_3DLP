...
#define _TEST_r (_TEST_m % _TEST_w)
    ...
    f2elm_t B[(_TEST_m / _TEST_w) * (POWER(_TEST_l, _TEST_w) / 2)];
	int label = 0;
	choose_base(_TEST_l, _TEST_m, _TEST_w, f[0], f[1], &label, B);
#if _TEST_r != 0
#define _TEST_solveDLP(x) (PH_DLP2((x), D, Str_, _TEST_m, _TEST_l, _TEST_w, B, f[label]))
#else
#define _TEST_solveDLP(x) (PH_DLP((x), D, Str_, _TEST_m, _TEST_l, _TEST_w, B))
#endif
	T_DLP(_TEST_l, _TEST_m, _TEST_w, B);
