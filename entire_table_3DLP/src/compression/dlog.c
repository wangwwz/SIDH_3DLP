...
void T_DLP(int l, int m, int w, f2elm_t* B)
{
	int lwd2 = sintpow(l, w) / 2;
	for (int k = 0; k < m / w; k++)
	{
		f2elm_t* Bk = &B[k * lwd2];
		for (unsigned int j = 1; j < sintpow(l, w) / 2; j++)
		{
			if (j % 2 == 1)
			{
				fp2copy(Bk[(j - 1) / 2], Bk[j]);
				sqr_Fp2_cycl(Bk[j], (digit_t*)&Montgomery_one);
				fp2correction(Bk[j]);
			}
			else
			{
				if (j % 3 == 2)
				{
					fp2copy(Bk[(j - 2) / 3], Bk[j]);
					cube_Fp2_cycl(Bk[j], (digit_t*)&Montgomery_one);
					fp2correction(Bk[j]);
				}
				else
				{
					fp2mul_mont(Bk[j - 1], Bk[0], Bk[j]);
					fp2correction(Bk[j]);
				}
			}
		}
...
void PH_DLP(f2elm_t h, int* D, int* Str, int m, int l, int w, f2elm_t* B)
{
	...
	while (k != m / w - 1)
	{
		...
		for (int ik = 0; ik <= top; ik++)
		{
			...
			{
				f2elm_t temp;
				fp2copy(B[stack[ik].i1 * lwd2 + a - 1], temp);
				if (sign == 1)
				{
					fpneg(temp[1]);
				}
...
void PH_DLP2(f2elm_t h, int* D, int* Str, int m, int l, int w, f2elm_t* B, f2elm_t g)
{
	int i = 0, j = 0, k = 0, yt2_sign = 0;
	int lwd2 = sintpow(l, w) / 2;
	int r = m % w;
	f2elm_t yt, yt1, yt2;
	f2elm_t temp;
		...
		for (int ik = 0; ik <= top; ik++)
		{
			if (a != 0)
			{
				if (ik == 0)
				{
					...
					{
						fp2copy(B[(stack[ik].i1 - 1)*lwd2+a - 1], temp);
						if (sign == 1)
						{
							yt2_sign = yt2_sign ^ sign;
							fp2mul_mont(yt2, temp, yt2);
						}
						else
						{
	...
	if (ar != 0)
	{
		fp2copy(B[(m / w - 2)*lwd2+ar - 1], temp);
		if (signr == 1)
		{
			yt2_sign = yt2_sign ^ signr;
			fp2mul_mont(yt2, temp, yt2);
		}
		else
		{
	...
	fpneg(yt2[1]);
	fp2mul_mont(yt2, yt1, yt1);
	for (int ir = 0; ir < w - r; ir++)
	{
		if ((l & 1) == 0) {
	...
	fp2mul_mont(yt, yt1, yt);
	fp2mul_mont(h, yt, yt);
	fp2correction(yt);
	int al, signl;
	small_DLP(yt, l, w, Bslice, &al, &signl);
...
void choose_base(int l, int m, int w, f2elm_t g1, f2elm_t g2, int* label, f2elm_t* A)
{
	*label = 1;
	...
	{
		k += lwd2;
		fp2copy(A[k - lwd2], A[k]);
		for (int j = 0; j < w; j++)
		{
			if ((l & 1) == 0)
			{
				sqr_Fp2_cycl(A[k], (digit_t*)&Montgomery_one);
			}
			else
			{
				cube_Fp2_cycl(A[k], (digit_t*)&Montgomery_one);
			}
		}
		if (is_felm_zero(A[k][1]) && fp2equal(A[k][0], Montgomery_one))
		{
			*label = 0;
			break;
		}
