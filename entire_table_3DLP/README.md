# compressed_SIDH 3DLP with entire lookup table
This code base is an attachment of the manuscript "An Alternative Approach for Computing Discrete Logarithms in Compressed SIDH", utilizing several techniques to compute only 3 discrete logarithms with the entire lookup table during public-key compression. In particular, the code measures the time required for public-key compression (except the dual isogeny computation).
## Compile and benchmark
To compile the code, you can simply execute:

```sh
$ make
```

To test and benchmark our algorithms, execute:
...
```
If you want to change the number of benchmark loops, modify the macro `BENCH_LOOPS` in [`test_sidh.c`](tests/test_sidh.c).

## Major changes in the code
Algorithm 3, 4, 5, 6 are implemented in [`dlog.c`](src/compression/dlog.c), which are called by function `EphemeralKeyGeneration_A_new` and `EphemeralKeyGeneration_B_new` in [`sidh_compressed.c`](src/compression/sidh_compressed.c) to realize the public key compression process. The strategies used by the new algorithms is stored in [`strategy.h`](src/compression/strategy.h).
